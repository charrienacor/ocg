...
export const GET: RequestHandler = async (event) => {
	...
	// PRELIMINARIES ---------------------------------
	firstPage.drawText(data?.Preliminaries.Semester?? 'N/A', {
		x: 250,
		y: height / 2 + 379,
		size: 10,
	...
	firstPage.drawText(data?.Preliminaries.School_Year?? 'N/A', {
		x: 340,
		y: height / 2 + 379,
		size: 10,
		color: rgb(red, green, blue)
	...
	firstPage.drawText(data?.Preliminaries.Degree_Program?? 'N/A', {
		x: 379,
		y: height / 2 + 366,
		size: 10,
		color: rgb(red, green, blue)
